...
static vector<byte> LoadFile(const char *fileName)
{
...
static bool SaveFile(const char *fileName, const vector<byte> &fileData)
{
	FILE *file = fopen(fileName, "wb");
	size_t bytesWritten = 0;
	if (fileData.size() > 0)
	{
		bytesWritten = fwrite(&fileData[0], 1, fileData.size(), file);
	}
	fclose(file);
	return bytesWritten == fileData.size();
}

bool InjectNSF(const string &fileName, const string &nsfName, int songNumber)
{
	vector<byte> romFile = LoadFile(fileName.c_str());
	if (romFile.size() == 0) return false;
	vector<byte> nsfFile = LoadFile(nsfName.c_str());
	if (nsfFile.size() == 0) return false;
	vector<byte> result = InjectNSF(romFile, nsfFile, songNumber);
	if (result.size() == 0) return false;
	return SaveFile(fileName.c_str(), result);
}
